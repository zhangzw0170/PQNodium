...
> 本地 `doc/reference/` 中若保存的是 `fallback`/`preview` 文件，表示当前环境未能抓取原始文档，仅保留可追溯的替代来源或说明文件。
...
| GM/T 0002-2012 | SM4 分组密码算法 | 国密后端对称加密；本地仅存 `gmt-0002-2012.preview.md` |
| GM/T 0003-2012 | SM2 椭圆曲线公钥密码算法 | 国密后端签名与密钥交换；本地仅存 `gmt-0003-2012.preview.md` |
| GM/T 0004-2012 | SM3 密码杂凑算法 | 国密后端哈希；本地仅存 `gmt-0004-2012.preview.md` |

## Protocols & Specifications

| 名称 | 说明 | 在项目中的用途 |
|------|------|--------------|
| Noise Protocol Framework | 轻量级握手协议框架 | TCP 传输安全层握手 |
| Kademlia DHT | 分布式哈希表 | 去中心化节点发现 |
| GossipSub | 基于 gossip 的发布/订阅 | 群组消息广播（Phase 5+） |
| libp2p Circuit Relay v2 | 中继协议 | NAT 穿透 fallback（Phase 4+） |
...
| 随机数 | `rand` crate（非密码学用途）；密码学安全随机数由 PQC crate 自带 |
...
| Crate | 用途 | 仓库 |
|-------|------|------|
| `libp2p` (features: `quic`, `tcp`, `noise`, `yamux`, `kad`, `identify`, `ping`) | P2P 网络框架 | <https://github.com/libp2p/rust-libp2p> |
| `tokio` | 异步运行时 | <https://github.com/tokio-rs/tokio> |
| `tauri` (v2) | 桌面应用框架 | <https://github.com/tauri-apps/tauri> |
| `clap` | CLI 参数解析 | <https://github.com/clap-rs/clap> |
| `tracing` + `tracing-subscriber` | 结构化日志 | <https://github.com/tokio-rs/tracing> |
| `futures` | 异步工具 (StreamExt 等) | <https://github.com/rust-lang/futures-rs> |
| `anyhow` | 错误处理 (仅 binary crate) | <https://github.com/dtolnay/anyhow> |
...
| `x25519-dalek` | X25519 ECDH | <https://github.com/dalek-cryptography/x25519-dalek> | 与 ml-kem 组合实现 Hybrid KEM |
| `sha2` | SHA-256 (Hybrid KEM KDF) | <https://github.com/RustCrypto/hashes> | RustCrypto 维护 |
...
> **注意**: `rustpq`（Hybrid KEM）和 `qcomm-core`（PQ Triple Ratchet）的 GitHub 仓库已失效，不采用。Hybrid KEM 基于 `ml-kem` + `x25519-dalek` 自行组合；PQ Triple Ratchet 待 Phase 5+ 评估。
...
| Crate | 用途 | 仓库 |
|-------|------|------|
| `serde` | 序列化框架 (libp2p feature) | <https://serde.rs/> |
...
| 项目 | 说明 | 链接 |
|------|------|------|
| Signal Protocol | Double Ratchet / 群组加密参考；本地仅存 `signal-protocol.fallback.md` | <https://signal.org/docs/> |
